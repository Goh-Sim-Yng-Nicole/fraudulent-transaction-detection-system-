...
class FraudRulesEngine:
    ...
    async def _check_velocity(self, transaction: dict[str, object]) -> dict[str, object]:
        ...
        if customer_hour_count > settings.max_txn_per_hour:
            ...
            reasons.append(f"Exceeded hourly transaction count ({customer_hour_count}/{settings.max_txn_per_hour})")

        if customer_hour_amount > settings.max_amount_per_hour:
            flagged = True
            score += settings.scoring_velocity_amount_hour
            reasons.append(f"Exceeded hourly spend limit (${customer_hour_amount:.2f}/${settings.max_amount_per_hour:g})")

        if customer_day_count > settings.max_txn_per_day:
            flagged = True
            score += settings.scoring_velocity_count_day
            reasons.append(f"Exceeded daily transaction count ({customer_day_count}/{settings.max_txn_per_day})")
        ...
        return {"flagged": flagged, "score": score, "reasons": reasons, "factors": factors}
    ...
    def _check_amount(self, transaction: dict[str, object]) -> dict[str, object]:
        ...
        flagged = False
        score = 0.0
        factors: dict[str, object] = {"amount": amount, "currency": currency}

        if amount >= settings.suspicious_amount_threshold:
            flagged = True
            score += settings.scoring_suspicious_amount
            reasons.append(f"Amount ${amount:g} exceeds suspicious threshold (${settings.suspicious_amount_threshold:g})")
    ...
    def _check_card(self, transaction: dict[str, object]) -> dict[str, object]:
        ...
        card_last_four = str(metadata.get("cardLastFour") or metadata.get("card_last_four") or "").strip()
        card_type = str(transaction.get("cardType") or transaction.get("card_type") or "").strip()

        reasons: list[str] = []
        flagged = False
        score = 0.0
        factors: dict[str, object] = {"cardLastFour": card_last_four or None, "cardType": card_type or None}
    ...
    def _check_time(self, transaction: dict[str, object]) -> dict[str, object]:
        ...
        return {"flagged": False, "score": score, "reasons": [], "factors": factors}
