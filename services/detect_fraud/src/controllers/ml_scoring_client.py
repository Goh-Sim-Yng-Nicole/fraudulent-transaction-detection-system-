...
from services.detect_fraud.src.utils.circuit_breaker import CircuitBreaker
...
def _normalize_response(data: dict[str, object] | None, fallback_score: int) -> dict[str, object]:
    if not data:
        return {
            ...
            "features": {"fallbackReason": "service_unavailable", "derivedFromRules": True},
        }
    ...
    return {
        ...
        "confidence": None,
        "modelVersion": "fallback-v1",
        "features": {"fallbackReason": "invalid_response", "derivedFromRules": True},
    }
...
class MlScoringClient:
    ...
    async def score(self, transaction: dict[str, object], rule_results: dict[str, object]) -> dict[str, object]:
        ...
        modern_payload = {
            ...
            "ruleResults": {
                "riskFactors": {
                    "velocity": {"countLastHour": count_last_hour},
                    "geography": {"highRiskCountry": high_risk_country},
                },
            },
        }
