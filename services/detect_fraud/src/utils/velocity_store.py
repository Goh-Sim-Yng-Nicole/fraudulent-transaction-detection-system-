...
class VelocityStore:
    ...
    def _summarize_window(
        ...
        *,
        ...
        distinct_recipients = {str(entry["recipient"]) for entry in hour_entries if entry.get("recipient")}
        distinct_merchants = {str(entry["merchant"]) for entry in hour_entries if entry.get("merchant")}
        merchant_entries = [
            entry for entry in hour_entries if merchant_key and entry.get("merchant") == merchant_key
        ]
        recipient_seen_before = bool(
            recipient_key and any(entry.get("recipient") == recipient_key for entry in hour_entries[:-1])
        )
        merchant_seen_before = bool(
            merchant_key and any(entry.get("merchant") == merchant_key for entry in hour_entries[:-1])
        )
        ...
        return {
            ...
            "merchantAmountLastHour": float(sum(float(entry["amount"]) for entry in merchant_entries)),
    ...
    async def record(
        ...
        *,
        ...
        if redis_client is None:
            ...
            return self._summarize_window(entries=entries, recipient_key=recipient_key, merchant_key=merchant_key)
