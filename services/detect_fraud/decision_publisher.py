...
class DecisionPublisher:
    ...
    async def _send_to_outsystems(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "OutSystems decision handoff failed for transaction %s: %s",
                transaction["id"],
                exc,
            )
            return False
    ...
    def _normalize_external_decision(self, body: dict[str, Any]) -> dict[str, Any] | None:
        ...
        return {
            ...
            or payload.get("outcomeReason")
            or "Decision returned by OutSystems",
            "decisionFactors": payload.get("decisionFactors") or payload.get("factors") or {"source": "OUTSYSTEMS"},
            ...
            "decisionVersion": payload.get("decisionVersion") or payload.get("version") or "outsystems",
        }
    ...
    def _make_local_decision(self, fraud_analysis: dict[str, Any], transaction: dict[str, Any]) -> dict[str, Any]:
        ...
        confidence_adjustment = self._apply_confidence_adjustment(
            int(fraud_analysis["riskScore"]),
            fraud_analysis["mlResults"].get("confidence"),
        )
    ...
    def _check_certainty_auto_decline(self, adjusted_score: int, confidence: Any) -> dict[str, str] | None:
        ...
        if not isinstance(confidence, (int, float)):
            return None
        if (
            adjusted_score >= settings.threshold_certainty_decline_min_score
            and float(confidence) >= settings.threshold_certainty_decline_min_confidence
        ):
            return {
                "decision": "DECLINED",
                "reason": (
                    f"High-certainty fraud signal (score {adjusted_score}, confidence {float(confidence)}) "
                    "auto-declined"
                ),
    ...
    def _check_high_value(self, transaction: dict[str, Any]) -> dict[str, str] | None:
        ...
        if settings.threshold_high_value_auto_flag and amount >= settings.threshold_high_value_amount:
            return {
                "decision": "FLAGGED",
                "reason": f"High-value transaction (${amount:g}) requires manual review",
                "type": "HIGH_VALUE",
            }
        return None
    ...
    def _check_geography(self, transaction: dict[str, Any]) -> dict[str, str] | None:
        ...
        if country and country in settings.require_manual_review_countries:
            return {
                "decision": "FLAGGED",
                "reason": f"Transaction from high-risk country ({country}) requires manual review",
