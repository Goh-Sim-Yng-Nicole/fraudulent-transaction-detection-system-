...
class Settings:
    def __init__(self) -> None:
        ...
        self.kafka_brokers = _env_csv(
            "KAFKA_BROKERS",
            _env("KAFKA_BOOTSTRAP_SERVERS", "localhost:9092"),
        )
        ...
        self.kafka_flagged_topic = _env(
            "KAFKA_OUTPUT_TOPIC_FLAGGED",
            _env("TOPIC_TRANSACTION_FLAGGED", "transaction.flagged"),
        )
        self.kafka_finalised_topic = _env(
            "KAFKA_OUTPUT_TOPIC_FINALISED",
            _env("TOPIC_TRANSACTION_FINALISED", "transaction.finalised"),
        )
        ...
        self.threshold_certainty_auto_decline_enabled = _env_bool(
            "THRESHOLD_CERTAINTY_AUTO_DECLINE_ENABLED",
            False,
        )
        self.threshold_certainty_decline_min_score = _env_int("THRESHOLD_CERTAINTY_DECLINE_MIN_SCORE", 70)
        self.threshold_certainty_decline_min_confidence = _env_float(
            "THRESHOLD_CERTAINTY_DECLINE_MIN_CONFIDENCE",
            0.9,
        )
        ...
        self.threshold_high_value_amount = _env_float(
            "THRESHOLD_HIGH_VALUE_AMOUNT",
        ...
        self.require_manual_review_countries = [
            value.upper()
            for value in _env_csv(
                "REQUIRE_MANUAL_REVIEW_COUNTRIES",
                _env("HIGH_RISK_COUNTRIES", "NG,RU,CN,PK"),
            )
        ]
