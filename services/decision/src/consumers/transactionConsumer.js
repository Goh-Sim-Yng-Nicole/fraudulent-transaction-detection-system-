...
const legacyEventTypeMap = {
  'transaction.flagged': 'transaction.flagged.v1',
  'transaction.finalised': 'transaction.finalised.v1',
};

const normalizeLegacyScoredMessage = (data) => {
  if (data?.fraudAnalysis && data?.transactionId) {
    return data;
  }
  ...
  const riskScore = Number(data?.rules_score ?? data?.riskScore ?? data?.score ?? 0);
  return {
    transactionId,
    customerId: data.customerId || data.customer_id || null,
    correlationId: data.correlationId || data.correlation_id || transactionId,
    originalTransaction: data.originalTransaction || null,
    fraudAnalysis: {
      transactionId,
      customerId: data.customerId || data.customer_id || null,
      merchantId: data.merchantId || data.merchant_id || null,
      riskScore,
      flagged: riskScore >= config.thresholds.flagMin,
      ruleResults: {
        ruleScore: riskScore,
        reasons: data.reason ? [data.reason] : [],
      },
      mlResults: {
        score: Number(data.fraud_probability ?? 0),
...
const handleMessage = async ({ topic, partition, message, heartbeat }) => {
  ...
  try {
    ...
    data = normalizeLegacyScoredMessage(data);
    ...
    await heartbeat();
    const outputPayload = {
      event_type: legacyEventTypeMap[decisionResult.decision === 'FLAGGED' ? 'transaction.flagged' : 'transaction.finalised'],
      trace_id: transactionId,
      data: decisionResult.decision === 'FLAGGED'
        ? {
