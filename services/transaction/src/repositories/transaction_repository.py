...
def _map_row(
    ...
    *,
    ...
    record = {
        "transaction_id": str(row.get("transaction_id") or row.get("id")),
    ...
    if include_workflow_state:
        record["outbound_event_published_at"] = row.get("outbound_event_published_at")
        record["outbound_event_publish_attempts"] = row.get("outbound_event_publish_attempts", 0)
        record["outbound_event_last_error"] = row.get("outbound_event_last_error")
        record["correlation_id"] = row.get("correlation_id")
...
class TransactionRepository:
    ...
    async def find_by_idempotency_key(
        ...
        *,
        ...
        # idempotency_key column not present in this schema version
        return None
            ...
            result = await session.execute(
                text("""
                    INSERT INTO transactions (
                      transaction_id, customer_id, sender_name, recipient_customer_id, recipient_name,
                      merchant_id, amount, currency, card_type, country, hour_utc,
                      status, fraud_score, outcome_reason, created_at, updated_at
                    ) VALUES (
                      :transaction_id, :customer_id, :sender_name, :recipient_customer_id, :recipient_name,
                      :merchant_id, :amount, :currency, :card_type, :country, :hour_utc,
                      :status, :fraud_score, :outcome_reason, NOW(), NOW()
                    )
                    RETURNING *
                """),
                {
    ...
    async def list_by_customer(self, customer_id: str, direction: str = "all") -> list[dict[str, Any]]:
        ...
        async with self._sessionmaker() as session:
            if direction in {"all", "outgoing"}:
                result = await session.execute(
                    text("SELECT * FROM transactions WHERE customer_id = :customer_id ORDER BY created_at DESC"),
            ...
            if direction in {"all", "incoming"}:
                result = await session.execute(
                    text("""
                        ...
                        WHERE recipient_customer_id = :customer_id AND status = 'APPROVED'
        ...
        async with self._sessionmaker() as session:
            result = await session.execute(
                ...
                {"transaction_id": transaction_id, "status": status,
                 "fraud_score": fraud_score, "outcome_reason": outcome_reason},
            )
            row = result.mappings().first()
            await session.commit()
    ...
    async def mark_outbound_event_failed(self, transaction_id: str, error_message: str) -> dict[str, Any] | None:
        async with self._sessionmaker() as session:
            result = await session.execute(
                text("""
                    UPDATE transactions
                    ...
                    RETURNING *
                """),
                {"transaction_id": transaction_id, "error_message": error_message},
            )
            row = result.mappings().first()
            await session.commit()
        return _map_row(row) if row else None
