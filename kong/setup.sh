...
JWT_KEY="ftds-customer-service"
JWT_SECRET="ftds-dev-secret-change-in-production-32chars"
...
curl -s -o /dev/null -w "  HTTP %{http_code}\n" -X POST "$KONG/services/ftds/routes" \
  --data "name=ftds-auth" \
  --data "paths[]=/api/auth" \
...
curl -s -o /dev/null -w "  HTTP %{http_code}\n" -X POST "$KONG/consumers" \
  --data "username=ftds-customers"
curl -s -o /dev/null -w "  HTTP %{http_code}\n" -X POST "$KONG/consumers/ftds-customers/jwt" \
  --data "algorithm=HS256" \
  --data "key=$JWT_KEY" \
  --data "secret=$JWT_SECRET"

echo "[6/7] Enabling JWT plugin on protected routes..."
curl -s -o /dev/null -w "  HTTP %{http_code}\n" -X POST "$KONG/routes/ftds-protected/plugins" \
  --data "name=jwt"
